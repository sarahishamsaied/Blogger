...
export default function SignUp() {
  ...
  return (
    <div className='bg-black h-screen w-screen flex align-middle justify-center signUp'>
        ...
        <input onChange={(e)=>setPassword(e.target.value)} className=" mb-2 shadow appearance-none border rounded bg-transparent w-full hover:bg-white transition ease-in py-3 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="username" type="text" placeholder="at least 8 characters"/>
        <label className = "text-white">Confirm Password</label>
        <input onChange={(e)=>setConfirmPassword(e.target.value)} className=" mb-2 shadow appearance-none border rounded bg-transparent w-full hover:bg-white transition ease-in py-3 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="username" type="text" placeholder="confirm password"/>
        ...
        <div>
            <GoogleLogin clientId='523022653649-aqb0p9r9mouehrcgibmri3rcn3q83ujg.apps.googleusercontent.com' 
            buttonText='sign up'
            onSucess/>
        </div>    
        </motion.div>
            ...
            {interests.map((interest,index)=>{
                return <Card key={index} type={interest} id = {index}/>
            })}
            </div>
